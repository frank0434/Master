...
```{r}
path = "C:/Data/Master/Data/ApsimxFiles/20201205BaseSlurpGsmax50.db"
simsMaxG = autoapsimx::read_dbtab(path = path, "Report")
path = "C:/Data/Master/Data/ApsimxFiles/20201205BaseSlurp.db"
simsUcona= autoapsimx::read_dbtab(path = path, "Report")
# sims$SimulationID %>% unique() %>% length()
...
```{r}
path = "C:/Data/Master/Data/ApsimxFiles/20200517BaseSlurp.db"
sims = autoapsimx::read_dbtab(path = path, "Report")
...
dt <- DBI::dbReadTable(con, name = "Report", check.names = TRUE)
dbDisconnect(con)
dt %>% 
  ggplot(aes(Clock.Today, SWmm.1.))+
  geom_line()
