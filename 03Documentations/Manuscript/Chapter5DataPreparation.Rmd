...
dt %>% 
  ...
  # add labels
  geom_point(data = dt[y %in% label$V1], aes(x, y), size = 5) + 
  annotate("text", 
           x = dt[y %in% label$V1]$x, 
           y = dt[y %in% label$V1]$y - 20,label = c("local minimum","global minimum"),
           size = 5)
ggsave("../Manuscript/Figures/fig5.5.png", width = 5, height = 5)
```
