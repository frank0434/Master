...
newnames <- c("Seedling StoneFree", "Seedling VeryStony", 
              "Established StoneFree", "Established Stony", "Established VeryStony")
...
dt_kl_AD <- dt_kl_layer[order(Depth)][, .(Depth, `Seedling VeryStony`, `Established VeryStony`)]
dt_kl_AD <- dt_kl_AD %>% 
...
lapply(list_AD_models$graphs, function(x){
   name <- unique(x$data[,c("Experiment", "SowingDate", "Depth")]) %>%
    unlist() %>%
    paste(., collapse = "-")
  ggsave(filename = here::here("/Data/images", paste0(name, ".png")), plot = x, 
         dpi = 300)
})
```
...
```{r}
DUL_LL[Experiment == "AshleyDene" &
         SowingDate == "SD1" &
         Depth == 1]$SW.DUL

cp_1_est[name =="int_1"]$mean * 0.266
```
...
SW_DUL_LL <- SW_initials[DUL_LL_SDs, on = c("Experiment", "SowingDate", "Depth")]
...
```{r}
water[,.(Experiment, Clock.Today, `SW(1)`)] %>% 
...
water_long = melt(water[,..disered_cols],
