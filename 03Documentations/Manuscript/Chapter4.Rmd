...
plot_PreObs <- function(dt, col_obs, col_pre, color = "SowingDate"){
  base_p <- dt %>% 
...
base_p <- SWCprofile %>% 
  plot_PreObs(., col_obs = "Observed.SWCmm", col_pre = "Predicted.SWCmm")+
  theme_water()
...
stats <- sims_stats(SWCprofile, keys = c("Experiment"),
                    col_pred = "Predicted.SWCmm",
                    col_obs = "Observed.SWCmm")
stats_rs <- stats[, unlist(stats, recursive = FALSE), by = .(Experiment)]

anchors <- SWCprofile[, .(x = min(Observed.SWCmm, na.rm = TRUE) * 1.1,
                          y = max(Predicted.SWCmm, na.rm = TRUE) * 0.9),
                      by =.(Experiment)]
...
abline_p +
  ...
  geom_text(data = stats_rs, 
            ...
            inherit.aes = FALSE) +
  labs(y = paste0(gsub("\\.|mm", " ", pre_col), " (mm)"),
       x = paste0(gsub("\\.|mm", " ", obs_col), " (mm)"))
ggsave("Figures/sw_preobs.png", width = 10, height =5, dpi = 320)
```
...
plot_timecourse <- function(DT, var, unit, label){
  ...
  var_subset <- c("Experiment","SowingDate", "Clock.Today",
                  pre_col, obs_col)
  SWCprofile <- DT[Clock.Today >= "2011-06-25", 
                   ..var_subset]
  SWCprofile <- fix_SDorder(SWCprofile)
  # label and colour
  timestep_colors <- c(pre_color,obs_color)
...
pre_obs[,.(Clock.Today, Name, Predicted.ShootWt, Observed.ShootWt)
        ][, Name := gsub("SowingDate", "", Name)] %>% 
  ggplot(aes(Observed.ShootWt, Predicted.ShootWt)) +
  geom_point() +
  facet_wrap(~ Name, scales = "free")
```
...
sql <- 'SELECT * FROM Report INNER JOIN _Simulations ON Report.SimulationID = _Simulations.ID WHERE "Clock.Today" >="2011-07-01"'
rs <- dbSendQuery(conn, sql)
pre20110701 <- DBI::dbFetch(rs) %>% 
  as.data.table()

```

```{r}
sub_obs <- obs[Clock.Today >= "2011-07-01",.(Name, Clock.Today, SWCmm)]
sub_pre <- pre20110701[,.(Name, Clock.Today, SWCmm)]
dt <- merge(sub_pre, sub_obs, by = c("Name", "Clock.Today"))

dt[, Clock.Today:=as.Date(Clock.Today)] %>% 
  ggplot(aes(Clock.Today)) +
  geom_point(aes(y = SWCmm.y)) +
  geom_line(aes(y = SWCmm.x)) +
  facet_wrap(~ Name, scales = "free")
```
