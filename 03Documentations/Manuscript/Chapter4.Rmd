...
```{r}
DT_LAI <- merge(unique(p_LAI$data), cutDates[Clock.Today >= magicDate], all.x = TRUE,
                    by = c("Experiment", "SowingDate", "Clock.Today"))

p_LAI$data[cutDates[Clock.Today >= magicDate],
...
DT <- merge(unique(p_shootwt$data), cutDates[Clock.Today >= magicDate],
            all.x = TRUE,
            by = c("Experiment", "SowingDate", "Clock.Today"))
...
DT <- merge(unique(p_leafwt$data), cutDates[Clock.Today >= magicDate],
            all.x = TRUE,
            by = c("Experiment", "SowingDate", "Clock.Today"))
...
stats_stemWt <- obs_stem[!is.na(StemWt)
                         ][, unlist(lapply(.SD, norm_stats),
                                    recursive = FALSE), by = .(Experiment), .SDcols = "StemWt"]
stats_stemWt
p_stemwt$data[, unlist(lapply(.SD, norm_stats), recursive = FALSE),
              by = .(Experiment),
...
p_shoot <- abline_p +
  ...
  geom_text(data = stats_rs, 
            ...
            inherit.aes = FALSE) +
  geom_text(data = stats_rs, 
            aes( x = x,  y = y - 250, label = RMSE_str), parse = TRUE,
            inherit.aes = FALSE) +
  labs(y = paste0(gsub("\\.|mm", " ", pre_col), var_unit_lab),
       x = paste0(gsub("\\.|mm", " ", obs_col), var_unit_lab))
```
...
p_aboveground <- plot_grid(p_shoot, p_leaf, p_stem, ncol = 1)
#ggsave("Figures/aboveground_preobs.png",plot = p_aboveground, 
       # width = 10, height =14, dpi = 320)
```
...
p_height <- abline_p +
  ...
  geom_text(data = stats_rs, 
            ...
            inherit.aes = FALSE) +
  geom_text(data = stats_rs, 
            aes( x = x,  y = y - 5, label = RMSE_str), parse = TRUE,
            inherit.aes = FALSE) +
  labs(y = paste0(gsub("\\.|mm", " ", pre_col), var_unit_lab),
       x = paste0(gsub("\\.|mm", " ", obs_col), var_unit_lab))
p_height
...
p_MS.node.No <- abline_p +
  ...
  geom_text(data = stats_rs, 
            ...
            inherit.aes = FALSE) +
  geom_text(data = stats_rs, 
            aes( x = x,  y = y - 1, label = RMSE_str), parse = TRUE,
            inherit.aes = FALSE) +
  labs(y = paste0(gsub("\\.|mm", " ", pre_col), var_unit_lab),
       x = paste0(gsub("\\.|mm", " ", obs_col), var_unit_lab))
p_MS.node.No
...
p_GrowthStage <- abline_p +
  ...
  geom_text(data = stats_rs, 
            ...
            inherit.aes = FALSE) +
  geom_text(data = stats_rs, 
            aes( x = x,  y = y - 0.25, label = RMSE_str), parse = TRUE,
            inherit.aes = FALSE) +
  labs(y = paste0(gsub("\\.|mm", " ", pre_col), var_unit_lab),
       x = paste0(gsub("\\.|mm", " ", obs_col), var_unit_lab))
p_GrowthStage
...
#ggsave("Figures/phenology_preobs.png",plot = p_phenology, 
       # width = 10, height = 9.5, dpi = 320)
```
#### Need the rainfall
