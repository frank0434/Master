...
var_subset <- c("Experiment","SowingDate", "Clock.Today",
                pre_col, obs_col)
SWCprofile <- pre_obs[Clock.Today >= magicDate, 
                      ..var_subset]
...
plot_timecourse <- function(DT, var, unit, label){
  ...
  var_subset <- c("Experiment","SowingDate", "Clock.Today",
                  pre_col, obs_col)
  DT_sub <- DT[Clock.Today >= magicDate, 
                   ..var_subset]
...
##QC CHECK
# pre_obs[Clock.Today >= magicDate, .(Experiment, SowingDate, Clock.Today,Predicted.ShootWt, Observed.StemWt)] %>% 
