...
sowingDates <- sowingDates[SowingDate%in% paste0("SD", 1:5),
...
data_SW <- read_Sims(path = path_richard)
id_vars <- readRDS(here("_targets/objects/id_vars"))
value_vars <- readRDS(here("_targets/objects/value_vars"))
SW_mean_new <- colwise_meanSW(DT = data_SW, 
                              id.vars = id_vars, 
                              col.vars = value_vars)
sw_initials_RESET <- initialSWC(sw_mean_new, sowingDates, id_vars)
...
# sw_initials
# sw_initials <- readRDS(here::here("_targets/objects/SW_initials_reset_Iversen12_SD1"))
...
```{r}
# Sometime the DUL is smaller than SW, this is gonna be a problem in apsimx 
# the extra initial soil water will lost quickly, which is not exact right.
# Fix this by adjusting the DUL to its 95%
...
keySoilParams[missDULlayers, ':=' (i.SW.DUL = i.SW.DUL/0.95,
                                   i.SW.LL = i.SW.LL/0.95)]
# Fiddle with labelling 
Layer = ifelse(1:22 == 1, 20, (1:22 + 1) * 10)

# Too crowed if label all, chose the odd position to label
pos <- 1:22 %% 3 == 1

keySoilParams %>% 
  ggplot(aes(x = Depth))+
  geom_col(aes(y = SW, fill = "SWC"), position = "dodge", width = 1)+
  geom_line(aes(y = i.SW.DUL, color = "DUL"), size = 1) +
  geom_point(aes(y = i.SW.DUL, color = "DUL"))+
  geom_line(aes(y = i.SW.LL, color = "LL"), size = 1)+
  geom_point(aes(y = i.SW.LL, color = "LL")) +
  # geom_line(aes(y = SW))+
  scale_x_reverse(breaks = (1:22)[pos], labels = Layer[pos], name = "Depth (cm)") +
  scale_y_continuous(expand = c(0, 0), 
                     name = expression("Volumetric water content"~ (mm^3 ~ mm^{-3}))) +
  coord_flip() +
  # geom_ribbon(aes(ymin = SW.LL, ymax = SW.DUL), fill = "grey80") +
...
ggsave("Figures/ch4_initialSW.png", width = 10, height =14, dpi = 320)
...
what were the absolute values for profile initial swc? what were the absolute values for profile initial DUL AND CLL?
...
**Rotation.No** is for the cutting dates. Need to extract the last date of a rotation.no.
...
timestep_p1 <- plot_timecourse(SWCprofile[Experiment == "AshleyDene"], 
                               variable, var_unit, var_unit_lab)

timestep_p2 <- plot_timecourse(SWCprofile[Experiment == "Iversen12"], 
                               variable, var_unit, var_unit_lab)
```
...
p1 <- timestep_p1 +
  geom_hline(data = DULmm[Experiment != "Iversen12"], aes(yintercept = DULmm), linetype = 2, size = 0.5, 
             alpha = 0.6)
p2 <- timestep_p2 +
  geom_hline(data = DULmm[Experiment == "Iversen12"], aes(yintercept = DULmm), linetype = 2, size = 0.5, 
             alpha = 0.6)
p_water <- plot_grid(p1, p2 +
                       theme(
                             axis.title.y = element_blank()
                             ), align = "hv",
                     ncol = 2)

ggsave("Figures/ch4_sw_timestamp.png",plot = p_water, width = 10, height =14, dpi = 320)
```

```{r}
p2$data[SowingDate == "SD6"] %>% 
  qplot(Clock.Today, Predicted.SWCmm, data = .) %>% 
  plotly::ggplotly()
```

SD6 in Iversen12 extracted `r 747-588` mm water from 2011-12-19 to 2012-02-29 rainfall recharged the SWC from 588 to 619
...
p_LAI_1 <- plot_timecourse(pre_obs[Clock.Today >= magicDate & 
                                     (Experiment != "Iversen12")], variable, var_unit, var_unit_lab)
p_LAI_2 <- plot_timecourse(pre_obs[Clock.Today >= magicDate & 
                                     (Experiment == "Iversen12")], variable, var_unit, var_unit_lab)
p <- plot_grid(p_LAI_1, p_LAI_2 +
                       theme(
                             axis.title.y = element_blank()
                             ), align = "hv",
                     ncol = 2)

ggsave("Figures/ch4_LAI_timestamp.png",plot = p_LAI, width = 10, height =14, dpi = 320)
...
pre_obs[DefoliationMethod[Clock.Today >= magicDate],
...
p_shootwt <- plot_timecourse(pre_obs[Clock.Today >= magicDate], variable, var_unit, var_unit_lab)

# Not worth to glue 
# p_1 <- plot_timecourse(pre_obs[Clock.Today >= magicDate & 
#                                      (Experiment != "Iversen12")], variable, var_unit, var_unit_lab)
# p_2 <- plot_timecourse(pre_obs[Clock.Today >= magicDate & 
#                                      (Experiment == "Iversen12")], variable, var_unit, var_unit_lab)
# p <- plot_grid(p_1, p_2 +
#                  theme(
#                    axis.title.y = element_blank()
...
ggsave("Figures/ch4_shoot_timestamp.png",plot = p_shootwt, width = 10, height =14, dpi = 320)
...
p <- plot_grid(p_1, p_2 +
               ...
               ncol = 2)
ggsave("Figures/ch4_leaf_timestamp.png",plot = p, width = 10, height =14, dpi = 320)
...
ggsave("Figures/ch4_height_timestamp.png",plot = p_height, width = 7, height = 5, dpi = 320)
...
ggsave("Figures/ch4_node_timestamp.png",plot = p_node, width = 7, height = 10, dpi = 320)
...
base_p <- DT_sub %>%
  ...
  theme_water() +
  geom_point(aes(color = SowingDate, shape = SowingDate, fill = SowingDate), size = 3)+
...
ggsave("Figures/ch4_swc_preobs.png", width = 10, height =5, dpi = 320)
```
...
abline_p +
  ...
  labs(y = paste0(gsub("\\.|mm", " ", pre_col), var_unit_lab),
       x = paste0(gsub("\\.|mm", " ", obs_col), var_unit_lab))
ggsave("Figures/ch4_LAI_preobs.png", width = 10, height =5, dpi = 320)
```
...
```{r}
legends <- get_legend(p_shoot)
p_aboveground <- plot_grid(p_shoot+theme(legend.position = "none"),
                           p_leaf + theme(legend.position = "none"), ncol = 1)
p_aboveground <- plot_grid(p_aboveground, legends, rel_widths = c(1, 0.2))
ggsave("Figures/ch4_aboveground_preobs.png",plot = p_aboveground,
...
DT_sub <- fix_SDorder(DT_sub)
colnames(DT_sub)
DT_sub[, .(max(Predicted.Height), max(Observed.Height, na.rm = TRUE)), by = .(Experiment)]
base_p <- DT_sub[!is.na(Observed.Height)] %>% 
  plot_PreObs(., col_obs = obs_col, col_pre = pre_col)+
  theme_water() +
  geom_point(aes(color = SowingDate, shape = SowingDate, fill = SowingDate), size = 3)+
  scale_shape_manual(values = 15:25)+
...
p_height <- abline_p +
  ...
  labs(y = paste0(gsub("\\.|mm", " ", pre_col), var_unit_lab),
       x = paste0(gsub("\\.|mm", " ", obs_col), var_unit_lab))
p_height
ggsave("Figures/ch4_height_preobs.png",plot = p_height, width = 10, height =5, dpi = 320)
```
...
p_MS.node.No <- abline_p +
  ...
  labs(y = paste0(gsub("\\.|mm", " ", pre_col), var_unit_lab),
       x = paste0(gsub("\\.|mm", " ", obs_col), var_unit_lab))
p_MS.node.No
ggsave("Figures/ch4_MSNN_preobs.png",plot = p_MS.node.No, width = 10, height =5, dpi = 320)
...
Fw <- fix_SDorder(Fw)
 Fw %>% 
    ...
    labs(y = "Ratio of leaf transpiration and water demand", 
         x = "Date") +
    scale_colour_manual(name = "col", values = timestep_colors, labels = timestep_labels)
ggsave("Figures/ch4_fw.png",
       width = 10, height = 10, dpi = 320)
```

```{r}
Fw$Clock.Today %>% range()
# Spring 
Fw[ round(Lucerne.Leaf.Fw,digits = 2) < 1.0 & Experiment == "AshleyDene" &  Clock.Today < "2011-12-01"
...
Fw[ Experiment == "AshleyDene" & round(Lucerne.Leaf.Fw, digits = 2) <= 0.5 & 
          Clock.Today < "2012-03-01" & Clock.Today > "2011-10-31"
    ][, .N, by = .(Experiment, SowingDate)
      ][, list(range(N), mean(N), sd(N))]
# Autumn
Fw[ Experiment == "AshleyDene" & round(Lucerne.Leaf.Fw, digits = 2) < 1 & 
