...
joined[!is.na(Experiment)] %>% 
  ...
  theme_water() +
  scale_color_manual(name = "Rotation", values = c("red", "blue", "black"))
...
model <- lm(fw ~ `T/Td`, data = df)
summary(model)

df %>% ggplot(aes(`T/Td`, fw)) +
  geom_point(aes( color = SowingDate...3), size = 3) +
  geom_smooth(method = "lm")+
  theme_water() +
  scale_color_brewer(name = "Sowing Date", palette = "Spectral")
