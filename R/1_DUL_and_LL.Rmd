...
DT_VWC %>% 
  ggplot(aes(Clock.Today, relativeSW)) +
  geom_point() +
...
```{r}
AD <- sw_long[Stats == "mean" & Experiment == "AshleyDene" &
          Clock.Today %between% c("2011-04-01", "2011-10-31")]

AD[order(SW, decreasing = TRUE), .SD[1] , by = c(id_vars, "Depth")]

doDUL_LL_range(SW = sw_mean_new)
AD %>% 
  ggplot(aes(Clock.Today, SW, group = Depth)) +
  geom_point() +
  scale_x_date(date_breaks = "8 weeks") +
