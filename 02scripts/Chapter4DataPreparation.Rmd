...
obs_wide <- obs_long[!is.na(value) & !SimulationName %like% "Ashley"] %>% 
  dcast.data.table(SimulationName + Clock.Today ~ variable)
# Correct a typo in the simulation name col
obs_wide[SimulationName == "AshlydeenDefoliation", 
...
```
Possible duplicated columns 
Biomass_Wt, ShootWt, Shootbiomass
Plantpopulation, Shootpopulation 
SWC, Total_SW

**Assume that all biomass unit is KG/HA** 
Not sure about the `Biomass_Wt` column in `GattonDefoliation`  
...
3. `Total_SW` is from `JimbourSowingDate*`. pretty high, left untouched. 

```{r}
obs_wide[, ':='(Shootbiomass = NULL, 
                Shootbiomasserror = NULL)]
...
#### Aggreate Richard's data into the obsalldata excel 
...
_Note_
Richard excel file has mix variable types. 
...
```{r}
idvars <- c("Site", "Date", "Season", "Sowing.Date", "Rep", "Plot",  
            "Seed", "Rotation.No.", "Harvest.No.",  "DAS")
valvars <- c("Total.DM", "Leaf.DM", "Stem.DM")
