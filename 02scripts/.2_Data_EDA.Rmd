...
water[Experiment == "AshleyDene" ] %>% 
  ...
  # geom_point()+
  geom_boxplot(outlier.colour = "#ff0000", outlier.size = 5) +
...
apsimx is not working properly in the bash env.  
...
#      variable.name = "Depth",value.name = "SW")
SW_mean[SowingDate == "SD9" & Experiment == "AshleyDene"] %>% 
  ggplot(aes(Clock.Today, `SW(10)`)) +
  geom_point() +
...
```{r}
artificial <- copy(water_mean)[variable %in% paste0("SW(", 10,")") 
                               & SowingDate %in% paste0("SD", 6:10) 
                               & Clock.Today >= "2011-10-21" 
