...
swcTovwc <- 100
ADSD1 <- water[Experiment == "AshleyDene" & SowingDate== "SD1"][,SWC:=NULL]
ADSD1
...
id_vars
max_min <- function(x){list(DUL = max(x, na.rm = TRUE)/swcTovwc,
                            LL = min(x, na.rm = TRUE)/swcTovwc)}

p <- ADSD1 %>% 
  melt(id_vars) %>% 
  # .[, variable := fct_reorder(variable, paste("SW(",1:23, ")"))] %>% 
...
nested_water$SWC_DUL_LL <- lapply(nested_water$SWC_list, function(x){
  x <- x[, lapply(.SD, mean),
         ...
         .SDcol = value_vars
           ...
           .SDcol = value_vars]
  x <- melt(x, variable.factor = FALSE)
  })
