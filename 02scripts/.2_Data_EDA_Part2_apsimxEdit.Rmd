...
```{r}
files = list.files("c:/Data/Master/03processed-data/bestfitLayerkl/", pattern = "*.db$", full.names = TRUE)[1:18]
...
```{r}
filenames = list.files("c:/Data/Master/03processed-data/apsimxFilesLayers/", pattern = "*.db$", full.names = TRUE)
...
for( i in filenames){
  ...
  # Rename the bestfit layer 2 
  newname = gsub("(.+)(_LayerKL_)(\\d{1,2}_)(\\d.\\d{2,4})(RFV_\\d{2})(\\.db$)",paste0("\\1\\2",layerno.,"_", bestkl, "\\5.apsimx"), basename(filename))
  filenameApsimx = gsub("\\.db$", ".apsimx", basename(filename))
  newname = file.path(sims_path, newname)
  oldname = file.path(sims_path, filenameApsimx)
  system(paste("cp", oldname, newname))
