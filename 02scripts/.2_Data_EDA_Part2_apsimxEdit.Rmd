...
```{r}
files = list.files("c:/Data/Master/03processed-data/bestfitLayerkl/", pattern = "*.db$", full.names = TRUE)
...
for( i in files){
  ...
  newname = gsub("(.+)(LayerKL_)(\\d.\\d{1,4})(RFV_\\d{1,2})(\\.db$)",paste0("\\1\\2",layerno.,"_", bestkl, "RFV_", bestRFV, ".apsimx"), basename(filename))
...
```{r}
filenames = list.files("c:/Data/Master/03processed-data/apsimxFilesLayers/", pattern = "*21.*.db$", full.names = TRUE)
sims_path = "c:/Data/Master/03processed-data/apsimxFilesLayers/"
for( i in filenames){
...
apsimxs = list.files("../03processed-data/apsimxFilesLayers/", pattern = "*.apsimx$")

DT = data.table(names = apsimxs)
layer1 = DT[, tstrsplit(names, split = "_|\\.")
   ][V2 == 0]
layer2to21 = DT[, tstrsplit(names, split = "_|\\.")
   ][V2 != 0]
layer1[,V2:=1]
layer1[, V6:=NULL]
layer2to21[, V3:= NULL]
setnames(layer2to21, colnames(layer2to21), paste0("V", 1:5))
DT = rbindlist(list(layer1, layer2to21), use.names = TRUE)
DT[, ':='(V1 = gsub("LayerKL", "", V1),
          V2 = as.numeric(V2),
          V3 = as.numeric(gsub("(\\d{1,4})(RFV)", "0.\\1", V3)),
          V4 = as.numeric(V4), 
          V5 = NULL)]
