...
```{python}
# LAI = LAI_Height.groupby(['Experiment', 'Clock.Today','SowingDate']).mean()
LAI_Height
```
...
LAIDaily = LAI.reindex(ThermalTimeAccum.index)
LAIDaily.loc[:, 'AccumTT'] = ThermalTimeAccum
# CoverDataDaily.loc[:,'AccumTT'] = ThermalTimeAccum
```

```{python}
print_full(LAIDaily.filter(regex='SD5'))
```
...
### 20200726 Re output LAI as the input of slurp
...
for site in sites: 
    for i in SDs:
        # Build a slicer
        idx = pd.IndexSlice
        # Slice out the right cols and site and sowing date, remove index
        LAI_SiteSD = LAI.loc[idx[site,:,i], 'LAImod'].reset_index()
        LAI_SiteSD.loc[:,['Clock.Today', 'LAImod']].\
...
# Graph to check the data 
LIGroupedMeanI12Forced.loc[:,
                              [X for X in LIGroupedMeanI12Forced.columns if X != (   'AccumTT',     '')]].\
