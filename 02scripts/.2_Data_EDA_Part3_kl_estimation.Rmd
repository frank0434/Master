...
```{r}
db <- "../03processed-data/apsimxFilesLayers/SKL_0.08RFV_20Iversen12SD1L2kl0.01.db"
...
pred_obs %>% 
  ggplot(aes(Date)) +
  geom_point(aes(y = `SW(2)`), col = "red")+
  geom_point(aes(y = `i.SW(2)`))
```
...
```{r}
DT = readd(SW_mean)
autoapsimx::sims_stats_multi(path_sims = "../03processed-data/apsimxFilesLayers/", pattern = "^SKL.+.db$", DT_observation = DT, mode = "Manual")
```
