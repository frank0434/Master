...
```{r}
pred_swc_SD2 <- report[,.(Date = as.Date(Clock.Today), SimulationID,KLR, RFV, SKL, k, LAI, PSWC)][order(SimulationID)]
```
...
ct = 1
for (i in unique(SW_mean$Experiment)){
  for (j in unique(SW_mean$SowingDate)) {
    
    cat(i , j , "\r\n")
    cols <- c("CheckpointID", "Experiment", "FolderName","Clock.Today","SowingDate", layer)
    SW_obs = readd(SW_mean)
    SW_obs = SW_obs[SowingDate == j & Experiment == i , colnames(SW_obs) %in% cols, with = FALSE]
    DT_stats = readd(DT_stats)
    DT = DT_stats[SowingDate == j & Experiment == i]
    
    
    DT = DT[, unlist(data, recursive = FALSE), by = .(Experiment,SowingDate, SimulationID, KLR, RFV, SKL)]
    DT_merged = merge(DT, SW_obs, by = c("Experiment","Clock.Today","SowingDate"))
    # Join two layers 
    stats = autoapsimx::sims_stats(DT_merged, col_pred = "SW(1).x", col_obs = "SW(1).y")
    stats_unnested = stats[, unlist(stats, recursive = FALSE), by = .(Experiment, SowingDate, SimulationID, KLR, RFV,SKL)]
    
    # maxNSE = max(stats_unnested$NSE)
    # minRMSE = min(stats_unnested$RMSE)
    # maxR2 = max(stats_unnested$R2)
    # zeroPbias = 0.0
    # best_fit = stats_unnested[NSE == maxNSE & RMSE == minRMSE & R2 == maxR2 & `PBIAS %`==zeroPbias]
    stats_sorted = stats_unnested[J(0), on = "PBIAS %", roll = "nearest"]
    setorder(stats_sorted, -NSE, -R2, RMSE)
    
    pred_obs_best = DT_merged[stats_sorted, on = c("Experiment", "SowingDate", "SimulationID", "KLR", "RFV","SKL")
                              ][, BestComb := paste0("SKL = ", SKL,
                                                     " KLR = ", KLR,
                                                     " RFV = ", RFV)]
    pred_obs_best[, beststats := paste0("NSE = ", NSE,
                                        " R.square = ", R2,
                                        " RMSE = ", RMSE)]
    # IF i or j is invalid -> skip
    if(isTRUE(dim(pred_obs_best)[1] == 0)){
      stats_sorted
      
    } else{
      P = pred_obs_best %>% 
        ggplot(aes(`SW(1).x`, `SW(1).y`)) + 
        geom_point() +
        facet_wrap( ~ BestComb + beststats) + 
        geom_abline(color = "red") + 
        theme_water() +
        # geom_text(aes(x = mean(pred_obs_best[["SW(1).x"]], na.rm = TRUE), 
        #               y = mean(pred_obs_best[["SW(1).y"]], na.rm = TRUE),
        #               vjust = 0,
        #               label = paste0("NSE = ", NSE,
        #                              "\r\nR.square = ", R2,
        #                              "\r\nRMSE = ", RMSE)),inherit.aes = FALSE) +
        labs(x = "Observation", y = "Predication", title = paste(i, j))
      l[[ct]] = P
    }
...
# stats[SimulationID ==4]
l[[10]]$data
# l[[10]]
l
```
## Strip off the bset skl and klr and rfv for the top layer
```{r}
l_bestcomb = lapply(l, function(x){
  DT = unique(x$data[,.(Experiment, SowingDate, KLR, SimulationID, SKL,RFV, NSE, R2, RMSE,`PBIAS %`)])
  DT
})
bestcomb = rbindlist(l_bestcomb)
fwrite(bestcomb, "../03processed-data/20200821bestcomb.csv")
melt(bestcomb, 
     id.vars = c("Experiment", "SowingDate", "SimulationID","NSE", "R2", "RMSE","PBIAS %")) %>% 
  ggplot(aes(variable, value)) +
  geom_point() +
  facet_wrap( ~ Experiment)
unique(bestcomb[,.(Experiment, SKL)]) %>% 
  ggplot(aes(Experiment)) +
  geom_point(aes(y = SKL))+
  geom_boxplot(aes(y = SKL), alpha = 0.5) 
unique(bestcomb[,.(Experiment, RFV)]) %>% 
  ggplot(aes(Experiment)) +
  geom_point(aes(y = RFV))
  # geom_boxplot(aes(y = RFV), alpha = 0.5) 
```
