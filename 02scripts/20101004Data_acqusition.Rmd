...
biomass <- dbReadTable(con, "biomass") %>% 
  as.data.table()
water <- dbReadTable(con, "SoilWater") %>% 
  as.data.table()
dbDisconnect(con)
...
water_sum <- copy(water)[, Data:=NULL
                         ][, SWC.0.1:= SWC.0.1 + SWC.0.2
                           ][, lapply(.SD, function(x) mean(x, na.rm = TRUE)), # calculate the mean 
                             .SD = value_vars, by = group_vars]
# Rename the layers 
water_sum[, SWC.0.2 := NULL]
swc_vars <- grep("SWC", colnames(water_sum), value = TRUE)

setnames(water_sum, swc_vars[-length(swc_vars)], paste0("SWC.", seq(0.1, 2.2, 0.1)))
# Stack two together
joined <- rbindlist(list(total_dm,water_sum), fill = TRUE)
# Modify in place. Type correction, SimName construction, 0 replace by NAs
joined <- joined[Sowing.Date %in% paste0("SD", 1:2)][, Date := as.Date(Date)
...
setnames(joined, c("Date","Total.DM", "SWC.2.3.m..mm."),c("Clock.Today","ShootWt", "SWC"))
joined[, ':=' (Site = NULL,
               Sowing.Date = NULL,
               Season = NULL)]
setcolorder(joined, "SimulationName")
...
?here::here
path_apX <- "G:/APSIM2019.10.10/Prototypes/Lucerne/"
