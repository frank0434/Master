...
tar_load(starts_with("opt.res"))
opt.res_AshleyDene_SD1
```
...
# Check the optimisiation results after 24 hours 

```{r}
tar_load(starts_with("opt.res"))
# load("C:/Data/Master/01Data/ProcessedData/apsimxFiles/2021-05-16opt.res.RData")
View(opt.res)
# plot(opt.res, plot.type = "bestmemit")
# plot(opt.res, plot.type = "storepop")
parameters

bestmemit <- as.data.table(opt.res_AshleyDene_SD1$member$bestmemit, keep.rownames = TRUE) %>% 
  melt.data.table(id.vars = "rn",variable.factor = FALSE)
storepop <- rbindlist(lapply(opt.res_AshleyDene_SD1$member$storepop, as.data.table, keep.rownames = TRUE), 
                      idcol = "itr") %>% 
...
base_p
# ggsave(filename = "03Documentations/20210516OptimisationDEMO.png",
       # base_p, dpi = 320, height = 7, width = 9)
```



# apsimx edit is not working 

```{r}
tar_load(path_apsimx)
path_apsimx

tar_load(apsimx_Basefile)
tar_load(apsimx_sims_dir)
apsimx_Basefile
apsimx_sims_dir
...
```{r}
# tar_visnetwork(targets_only = TRUE, allow = ends_with("SD1"), label = "time")
```


# load and check 
```{r}
tar_load(starts_with("Site"))
Sites_Iversen12_SD1
tar_load(starts_with("SD"))
SD_AshleyDene_SD1
...
input_list_Iversen12_SD10$DUL_LL_range_arbitrary_Iversen12_SD10
names(input_list_AshleyDene_SD1)
input_list_AshleyDene_SD1$obs_AshleyDene_SD1
input_list_AshleyDene_SD1$SW_initials_AshleyDene_SD1
input_list_Iversen12_SD10
```
# Debug sw initials
...
SW_initials_Iversen12_SD1
sowingDates %>% filter_SD(trts = c(
  "AshleyDene", "SD1"
))
actualSD_AshleyDene_SD1
actualSD_AshleyDene_SD2
id_vars
initialSWC(SW_sub_AshleyDene_SD1, 
           sowingDate = actualSD_AshleyDene_SD1, id_vars)


```
# Check reset swc
```{r}
tar_load(starts_with("SW_initials_reset"))
SW_initials_reset_AshleyDene_SD1
SW_initials_reset_Iversen12_SD1
tar_load(starts_with("SW_sub"))
tar_load(starts_with("reset"))
resetSD_AshleyDene_SD1
resetDates
resetSD_AshleyDene_SD1
...
# Test wrapper here
```{r}
parameters <- readRDS("_targets/objects/parameters")
wrapper_deoptim(parameters, paravals, obspara, 1, 1, 
                template,
                path_apsimx,                              
                magicDate,                              
                apsimx_Basefile,
