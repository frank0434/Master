...
tar_load("value_vars")
tar_load("id_vars")
doDUL_LL_range(SW = tar_read(SW_mean))
...
range_check(SW_mean, 1, boundry = "DUL")
range_check(SW_mean, 1, boundry = "LL")
...
sdcols <- grep("sd", colnames(SW_mean), value = TRUE)
neededcols <- c(tar_read(id_vars), sdcols)
DT_sd <- SW_mean[,..neededcols
...
p_initialSW <- SW_initials[SowingDate%in%paste0("SD",1:5) & Stats == "mean"] %>% 
...
SW_initials[SowingDate%in%paste0("SD",1:5) & Stats %in% c("mean", "sd")]
